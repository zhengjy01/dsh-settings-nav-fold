		...
		function apply(ctx) {
			...
			const sync = () => {
				...
				try {
					...
					let before = coreEls.length > 0 ? coreEls[coreEls.length - 1].nextSibling : navList.firstChild;
					for (const el of wanted) {
						const placed = el.parentNode === navList && (before === null ? el === navList.lastChild && el.nextSibling === null : el === before || el.nextSibling === before);
						if (!placed) navList.insertBefore(el, before);
						before = el;
					}
					// 4) 更新所有注入行文本
