...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			console.log("[settings-nav-organizer] v1.4.1 loaded");
			...
			const loadPersisted = () => {
				...
				return { groups, foldThirdParty };
			};
			const savePersisted = () => {
				try {
					localStorage.setItem(STORAGE_KEY, JSON.stringify({ groups: state.groups, foldThirdParty: state.foldThirdParty }));
				} catch (_ignored) {
			...
			const state = { folded: true, count: 0, groups: persisted.groups, open: {}, foldThirdParty: persisted.foldThirdParty };
			...
			const sync = () => {
				...
				try {
					...
					if (!state.foldThirdParty) {
						...
						for (const g of state.groups) {
							if (navList.dataset[`snavOpen${g.id}`] !== undefined) delete navList.dataset[`snavOpen${g.id}`];
						}
			...
			function ManagePage(props) {
				...
				// 渲染时动态获取宿主端管理服务：插件激活时 remote 可能尚未就绪，
				// 用户点开此页时必然已就绪；未就绪（宿主端未提供）时给出提示，
				// 不影响折叠/分组等核心功能
				const manageService = ctx.get("remote.settingsNavManage");
				if (manageService === undefined) {
					return React.createElement("div", { className: "dsh-snav-page" },
						React.createElement("p", { className: "dsh-snav-err" }, tPage("manageUnavailable")),
					);
				}
				const manage = () => manageService;
				const load = () => {
					manage()
						.list()
						.then((res) => {
				...
				const act = (moduleName, method, okText) => {
					setBusy(moduleName);
					manage()
						[method]({ moduleName })
						.then((res) => {
											...
											it.manageable
												? React.createElement(React.Fragment, null,
													busy === it.moduleName
														...
														: it.enabled
															? React.createElement("button", { className: "dsh-snav-btn dsh-snav-mini", onClick: () => act(it.moduleName, "setEnabled", tPage("opOk")) }, tPage("disable"))
															: React.createElement("button", { className: "dsh-snav-btn dsh-snav-mini", onClick: () => act(it.moduleName, "setEnabled", tPage("opOk")) }, tPage("enable")),
													confirmId === it.moduleName
														? React.createElement(React.Fragment, null,
															React.createElement("button", { className: "dsh-snav-btn dsh-snav-mini dsh-snav-danger", onClick: () => act(it.moduleName, "uninstall", tPage("opOk")) }, tPage("uninstallConfirm")),
